...
<template>
  <article
    class="rounded-md sm:cursor-pointer overflow-hidden relative h-20 bg-white p-6 border broder-gray-200 shadow-sm md:hover:shadow-md ease-in-out duration-200 flex items-center"
  >
    <div class="sm:basis-1/12 hidden sm:block">
      <img
        ...
        :src="blog.image"
        alt="Blog thumbnail"
        class="w-1/12 h-full absolute object-cover top-0 left-0"
      />
      <div
        v-else
        class="h-full w-1/12 absolute object-cover top-0 left-0 bg-gradient-to-tl from-cyan-500 to-blue-500"
      />
    </div>
